...
function TableCell({ children, addlClasses = "", ...props }) {
  return (
    ...
    <td class="p-4 border-b border-slate-200 py-5" {...props}>
      <p class="block font-semibold text-sm text-slate-800">{children}</p>
    </td>
  )
}
...
function TableSearchBar({ title, subTitle, searchPlaceholder, ...props }) {
  return (
...
export default function Table({ resourceName, headers, paginationCtx, rowRenderer, emptyMsg = "Empty" }) {
